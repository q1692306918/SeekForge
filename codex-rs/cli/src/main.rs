...
use codex_cli::run_login_guidance;
...
enum Subcommand {
    ...
    /// Show DeepSeek-native authentication guidance.
    Login(LoginCommand),
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        hide = true,
        help = "Deprecated: native API-key login is disabled in SeekForge"
    )]
    with_api_key: bool,
    ...
    #[arg(
        ...
        hide = true,
        help = "Deprecated: native access-token login is disabled in SeekForge"
    )]
    ...
    match subcommand {
        ...
        Some(Subcommand::Login(mut login_cli)) => {
            ...
            match login_cli.action {
                ...
                None => {
                    let attempted_native_login = login_cli.with_api_key
                    ...
                    run_login_guidance(login_cli.config_overrides, attempted_native_login).await;
                }
            }
        }
...
async fn load_exec_server_remote_auth_provider(
    ...
    anyhow::bail!(
        "SeekForge remote exec-server registration does not support native OpenAI/ChatGPT login, auth.json API-key login, or CODEX_API_KEY. Use --use-agent-identity-auth with CODEX_ACCESS_TOKEN."
    )
}
...
mod tests {
    ...
    #[tokio::test]
    async fn exec_server_remote_default_auth_rejects_native_login() -> anyhow::Result<()> {
        ...
        let error = load_exec_server_remote_auth_provider(&config, "https://example.com", false)
            .await
            .expect_err("default remote registration auth is unsupported");
        let error = error.to_string();

        assert!(error.contains(
            "SeekForge remote exec-server registration does not support native OpenAI/ChatGPT login"
        ));
        assert!(!error.contains("codex login"));
        assert!(error.contains("--use-agent-identity-auth"));
        assert!(error.contains("CODEX_ACCESS_TOKEN"));
